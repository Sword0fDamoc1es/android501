...
import com.example.meet4sho.R;
import com.example.meet4sho.Restaurant;
import com.example.meet4sho.RestaurantRecyclerAdapter;
import com.example.meet4sho.TM_RecyclerAdapter;
import com.example.meet4sho.Yelp_LocInfo;
import com.example.meet4sho.Yelp_LocList;
...
public class API_Fetch {
    ...
    private static final String YELP_API_KEY = "88RR4WnZJtAJeY_z4UA6bR-NhnlmhFk5zYFqxD30lDTrpfa543apNhHmDzum2ElLWI4od6pGjBfMjIoG7T9ReRU8vYaQejY57KwBwKwoj-r9ID9ahOJG4D1d5TRDYnYx";

    private static TM_RecyclerAdapter ra;
    private static List<String> names;
    private static List<String> descriptions;
    private static List<String> imgUrls;
    private static List<String> latitude;
    private static List<String> longitude;

    private static RestaurantRecyclerAdapter resAdapter;

    private static List<Restaurant> rests = new ArrayList<>();
    private static List<String> resInfo = new ArrayList<>();



    public static void Search(TM_RecyclerAdapter r, List<String> nms, List<String> des, List<String> urls, List<String> lg, List<String> lt, int pn, String city, String keyword){
        ra = r;
        names = nms;
        descriptions = des;
        imgUrls = urls;
    ...
    public static void RestaurantSearch(RestaurantRecyclerAdapter r, String cat, String sort, String lg, String lt){
        resAdapter = r;

        new YelpApiBusinessSearch().execute(cat, sort, lg, lt);
    }
            ...
            try {
                ...
                if (jsonObj != null) {
                    ...
                    for (int i=0; i < jsonArray_events.length(); i++) {
                        ...
                        longitude.add(jsonArray_events.getJSONObject(i).getJSONObject("_embedded").getJSONArray("venues").getJSONObject(0).getJSONObject("location").getString("longitude"));
                        latitude.add(jsonArray_events.getJSONObject(i).getJSONObject("_embedded").getJSONArray("venues").getJSONObject(0).getJSONObject("location").getString("latitude"));
            ...
            try {
                ...
                if (jsonObject != null) {
                    ...
                    for (int i = 0; i < restaurants.length(); i++) {
                        ...
                        resInfo.add(name);
                        resInfo.add(image_url);
                        resInfo.add(is_closed);
                        resInfo.add(url);
                        resInfo.add(review_count);
                        resInfo.add(rating);
                        resInfo.add(latitude);
                        resInfo.add(longitude);
                        resInfo.add(price);
                        resInfo.add(address1);
                        resInfo.add(address2);
                        resInfo.add(address3);
                        resInfo.add(city);
                        resInfo.add(zip_code);
                        resInfo.add(country);
                        resInfo.add(state);
                        resInfo.add(display_address);
                        resInfo.add(phone);
                        resInfo.add(display_phone);
                        resInfo.add(distance);
                        rests.add(new Restaurant(resInfo));
                    }
                    resAdapter.notifyData(rests);
                }
            }catch(JSONException e){
                    e.printStackTrace();
                }
